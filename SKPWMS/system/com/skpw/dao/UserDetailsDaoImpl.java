...
public class UserDetailsDaoImpl extends JdbcDaoSupport implements
		UserDetailsDao {
	private static final String SQL_USERS_UPDATE_LOCKED = "UPDATE T_SYS_USERINFO SET blockStatus = ? WHERE username = ?";
	private static final String SQL_USERS_COUNT = "SELECT COUNT(*) FROM T_SYS_USERINFO WHERE username = ?";
	private static final String SQL_USER_ATTEMPTS_GET = "SELECT FID  id,FUserName  username,FAttempts  attempts,FLastModified  lastmodified FROM T_SYS_UserAttempts WHERE FUserName  = ?";
	private static final String SQL_USER_ATTEMPTS_INSERT = "INSERT INTO T_SYS_UserAttempts(FUserName, FAttempts, FLastModified) VALUES(?,?,?)";
	private static final String SQL_USER_ATTEMPTS_UPDATE_ATTEMPTS = "UPDATE T_SYS_UserAttempts  SET FAttempts = FAttempts + 1, FLastModified = ? WHERE FUserName = ?";
	private static final String SQL_USER_ATTEMPTS_RESET_ATTEMPTS = "UPDATE T_SYS_UserAttempts  SET FAttempts = 0, FLastModified = null WHERE FUserName = ?";
