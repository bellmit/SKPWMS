...
@Repository("dayStatisticDao")
public class DayStatisticDaoImpl implements DayStatisticDao {

	public List<DayReportBean> dayStatisticReport(String monitortime, int ffacilityid,
			int controlid) {
		String SQL = "SELECT fhour,FControlerID ,FMonitorTime,FFacilityID, "
				...
				+ " FROM ("
				+ " SELECT T1.FHOUR,FFacilityID,FCONTROLERID ,FMONITORTIME,FPOLLUTANTID,FAVGVALUE,FDischarge,FZSAVGVALUE,FZsDischarge "
				+ " FROM  DM_HOUR  T1 "
				+ " LEFT JOIN  ( "
				+ " SELECT FHOUR,FFacilityID,FCONTROLERID ,FMONITORTIME,FPOLLUTANTID,FAVGVALUE,FDischarge,FZSAVGVALUE,FZsDischarge "
				+ " FROM  T_RT_WASTEGASHOUR "
				+ " WHERE   FMONITORTIME =?  AND  FCONTROLERID=?  and FFacilityID=?) T2 ON T1.FHOUR =T2.FHOUR)  T3 "
				+ " GROUP BY FHOUR,FCONTROLERID, FMONITORTIME,FFacilityID ";
		List<DayReportBean> list = new ArrayList<DayReportBean>();
		...
		try {
			...
			ps.setString(1, monitortime);
			ps.setInt(2, controlid);
			ps.setInt(3, ffacilityid);
			rs = ps.executeQuery();

			while (rs.next()) {
				DayReportBean reportBean = new DayReportBean();
				reportBean.setMonitertime(rs.getInt("Fhour") + "");
				reportBean.setCtrlid(rs.getInt("fcontrolerid"));				
				reportBean.setSO1Rtd(rs.getFloat("S01Rtd")> 0 ? rs.getFloat("S01Rtd") + "" : "-");
				reportBean.setSO2Rtd(rs.getFloat("S02Rtd")> 0 ? rs.getFloat("S02Rtd") + "" : "-");
				reportBean.setSO3Rtd(rs.getFloat("S03Rtd")> 0 ? rs.getFloat("S03Rtd") + "" : "-");
				reportBean.setSO4Rtd(rs.getFloat("S04Rtd")> 0 ? rs.getFloat("S04Rtd") + "" : "-");
				reportBean.setSO5Rtd(rs.getFloat("S05Rtd")> 0 ? rs.getFloat("S05Rtd") + "" : "-");
				reportBean.setSO6Rtd(rs.getFloat("S06Rtd")> 0 ? rs.getFloat("S06Rtd") + "" : "-");
				reportBean.setSO7Rtd(rs.getFloat("S07Rtd")> 0 ? rs.getFloat("S07Rtd") + "" : "-");
				reportBean.setSO8Rtd(rs.getFloat("S08Rtd")> 0 ? rs.getFloat("S08Rtd") + "" : "-");
				reportBean.setBO2RtdPfl(rs.getFloat("BO2Discharge")> 0 ? rs.getFloat("BO2Discharge") + "" : "-");	
				reportBean.setO1Rtd(rs.getFloat("O1Rtd")> 0 ? rs.getFloat("O1Rtd") + "" : "-");
				reportBean.setO1Rtdpfl(rs.getFloat("O1RtdFDischarge")> 0 ? rs.getFloat("O1RtdFDischarge") + "" : "-");
				reportBean.setO1ZSRtd(rs.getFloat("O1ZSRtd")> 0 ? rs.getFloat("O1ZSRtd") + "" : "-");
				reportBean.setO1ZSRtdPfl(rs.getFloat("O1FZsDischarge")> 0 ? rs.getFloat("O1FZsDischarge") + "" : "-");
				reportBean.setO2Rtd(rs.getFloat("O2Rtd") > 0 ? rs.getFloat("O2Rtd") + "" : "-");
				reportBean.setO2Rtdpfl(rs.getFloat("O2RtdFDischarge")> 0 ? rs.getFloat("O2RtdFDischarge") + "" : "-");
				reportBean.setO2ZSRtd(rs.getFloat("O2ZSRtd") > 0 ? rs.getFloat("O2ZSRtd") + "" : "-");
				reportBean.setO2ZSRtdPPfl(rs.getFloat("O2FZsDischarge")>0?rs.getFloat("O2FZsDischarge") + "" : "-");
				reportBean.setO3Rtd(rs.getFloat("O3Rtd") > 0 ? rs.getFloat("O3Rtd") + "" : "-");
				reportBean.setO3Rtdpfl(rs.getFloat("O3RtdFDischarge")> 0 ? rs.getFloat("O3RtdFDischarge") + "" : "-");
				reportBean.setO3ZSRtd(rs.getFloat("O3ZSRtd") > 0 ? rs.getFloat("O3ZSRtd") + "" : "-");
				reportBean.setO3ZSRtdPfl(rs.getFloat("O3FZsDischarge")>0?rs.getFloat("O3FZsDischarge") + "" : "-");				
				list.add(reportBean);
	...
	public List<DayReportBean> findMMAS(String monitortime,int ffacilityid, int controlid) {
		String SQL = "select  " 
				...
				+ " from ( SELECT fhour,FControlerID ,FMonitorTime,FFacilityID, "
				+ " MAX(CASE FPOLLUTANTID WHEN '13' THEN FAVGVALUE ELSE 0 END) AS S01Rtd,"
				+ " MAX(CASE FPOLLUTANTID WHEN '14' THEN FAVGVALUE ELSE 0 END) AS S02Rtd,"
				+ " MAX(CASE FPOLLUTANTID WHEN '15' THEN FAVGVALUE ELSE 0 END) AS S03Rtd,"
				+ " MAX(CASE FPOLLUTANTID WHEN '16' THEN FAVGVALUE ELSE 0 END) AS S04Rtd,"
				...
				+ " FROM  T_RT_WASTEGASHOUR "
				+ " WHERE   FMONITORTIME =?  AND  FCONTROLERID=?  and FFacilityID=?) T2 ON T1.FHOUR =T2.FHOUR)  T3 "
				+ " GROUP BY FHOUR,FCONTROLERID, FMONITORTIME,FFacilityID  ) t4  ";
		List<DayReportBean> list = new ArrayList<DayReportBean>();
		PreparedStatement ps = null;
		ResultSet rs = null;
		Connection conn = DBUtil.getConnection();
		try {
			...
			ps.setInt(3, ffacilityid);
			rs = ps.executeQuery();
			DayReportBean max = new DayReportBean();// 最大值
			DayReportBean min = new DayReportBean();// 最小值
			DayReportBean avg = new DayReportBean();// 平均值
			DayReportBean sum = new DayReportBean();// 总和
	...
	public static void main(String[] args) {
		DayStatisticDao dayStatisticDao = new DayStatisticDaoImpl();

		List list = dayStatisticDao.dayStatisticReport("2014-11-20",1, 39);
		
//		List list = dayStatisticDao.findMMAS("2014-11-20",39, 1);
