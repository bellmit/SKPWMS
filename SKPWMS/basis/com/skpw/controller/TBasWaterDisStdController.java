...
@Controller
public class TBasWaterDisStdController {
	...
	//分页查询
	@RequestMapping("/tBasWaterDisStdController/findByPage")
	public @ResponseBody Map<String, Object> findByPage(int page, int rows){
		//用户存储数据，转为json
		Map<String, Object> map = new HashMap<String, Object>();
		//总条数
		int total = tBasWaterDisStdService.queryCount();
		Pageable pageable = new PageRequest(page-1, rows);
		//查询出当前页面的信息
		List<TBasWaterDisStd> list = tBasWaterDisStdService.findByPage(pageable);
	...
	//保存
	@RequestMapping("/tBasWaterDisStdController/save")
	public void save(HttpServletResponse res, TBasWaterDisStd bean){
		String str = "true";
		try {
			
			try{
				String id = bean.getFwaterDisStdId();
				...
				tBasWaterDisStdService.save(bean);
			}catch(Exception e){
				str = "false";
				e.printStackTrace();
			}
	...
	//删除
	@RequestMapping("/tBasWaterDisStdController/del")
	public @ResponseBody Map<String, Object> del(String id){
		Map<String, Object> map = new HashMap<String, Object>();
		Boolean b = true;
		try{
			tBasWaterDisStdService.del(id);
		}catch(Exception e){
			b = false;
			e.printStackTrace();
		}
