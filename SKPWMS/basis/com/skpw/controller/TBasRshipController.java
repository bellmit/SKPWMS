...
@Controller
public class TBasRshipController {
	...
	//分页查询
	@RequestMapping("/tBasRshipController/findByPage")
	public @ResponseBody Map<String, Object> findByPage(int page, int rows){
		//用户存储数据，转为json
		Map<String, Object> map = new HashMap<String, Object>();
		//总条数
		int total = TBasRshipService.queryCount();
		Pageable pageable = new PageRequest(page-1, rows);
		//查询出当前页面的信息
		List<TBasRship> list = TBasRshipService.findByPage(pageable);
	...
	//保存
	@RequestMapping("/tBasRshipController/save")
	public void save(HttpServletResponse res, TBasRship bean){
		String str = "true";
		try {
			
			try{
				String id = bean.getFrshipId();
	...
	//删除
	@RequestMapping("/tBasRshipController/del")
	public @ResponseBody Map<String, Object> del(String id){
		Map<String, Object> map = new HashMap<String, Object>();
		Boolean b = true;
		try{
			TBasRshipService.del(id);
		}catch(Exception e){
			b = false;
			e.printStackTrace();
		}
